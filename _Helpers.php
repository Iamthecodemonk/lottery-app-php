...
class SessionService
{
	...
	public function startSession($userId, $email, $role = null, array $extra = [])
	{
		try {
			if (session_status() == PHP_SESSION_NONE) {
				session_start();
...
class JWT
{
	// JWT secret key (change to a secure value in production)
	private $jwtSecret = 'mysupersecretkey';
...
class ToolHelper
{
	...
	public function sendMail($to, $subject, $body, $from = 'blueextra_ @blueextralotto.com', $fromName = 'BlueExtra Lotto Admin', $embeddedImagePath = null)
	{
		...
		try {
			// Server settings
			$mail->isSMTP();
			$mail->Host = 'blueextralotto.com';
			$mail->SMTPAuth = true;
			$mail->Username = 'blueextra_ @blueextralotto.com';
			$mail->Password = 'R!jK#q4Waf9?';
